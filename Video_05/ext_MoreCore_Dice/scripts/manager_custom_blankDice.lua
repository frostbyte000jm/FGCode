...
function performAction(draginfo, rActor, sParams)
    ...
    aContainer.bRollin = true;      -- For accidental spam rolls

    -- aRoll Setup
    local aRoll = {};                   -- table (array/dictionary) to collect information about the roll.
    aRoll.aDice = {};                   -- table of dice
    aRoll.sDiceSides = "";              --- If dice are static, enter here, else capture in the dice parser
    aRoll.sType = sCmd;                 -- Dice Names
    aRoll.sDesc = "";                   -- roll Description
    aRoll.nMod = 0;                     -- Modifier (from dice string and Mod Stack)
    -- NOTE: I suggest using aContainer to pass data from one function to another.
    -- aRoll will convert all data to a string and lose any boolian data.
...
function onLanded(rSource, rTarget, rRoll)
    ...
    aContainer.bRollin = false; --To prevent accidental spam rolls.
end
