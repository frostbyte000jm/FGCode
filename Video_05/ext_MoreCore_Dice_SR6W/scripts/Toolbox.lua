...
--- ***  Nodes needed to pull data from Character Sheet or Combat Tracker ***
    ...
    for _,nodeActorEffects in pairs(nodeActorCombatTracker.getChild("effects").getChildren()) do
        ...
        if string.match(sLabel,"KeyWord:") then                     --- Update KeyWord
            ...
            aContainer.nKeyWord = tonumber(sValue);
            DB.deleteNode(nodeActorEffects); --fjkdslajf
        end
        nCount = nCount + 1;
    end
...
--- *** Exploding Dice ***
    ...
    aContainer.nExplodeValue = 0;       --- Value when dice explode. This can be stack or dynamic.
...
--- *** Revise Dice Total ***
    -- Add to onInit() Declarations
    aContainer.nMyTotal = 0;
    ...
    for _,aDice in ipairs(rRoll.aDice) do
        ...
        aContainer.nMyTotal = aContainer.nMyTotal + nResult;                      --- Add Logic here
        Debug.console("aContainer.nMyTotal: ",aContainer.nMyTotal);
    end
...
--- For Revising Total Field
function onDiceTotal()
    return true, aContainer.nMyTotal;
end
